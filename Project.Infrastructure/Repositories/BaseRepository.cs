...
namespace Project.Infrastructure.Repositories
{
    ...
    public class BaseRepository<T> : IBaseRepository<T> where T : class
    {
        ...
        private Expression<Func<U, object>> GetOrderByExpression<U>(string propertyName)
        {
            var parameter = Expression.Parameter(typeof(U), "x");
            var property = Expression.Property(parameter, propertyName);
            var conversion = Expression.Convert(property, typeof(object));

            return Expression.Lambda<Func<U, object>>(conversion, parameter);
        }
        ...
        public virtual async Task<T> GetById<Tid>(List<Expression<Func<T, object>>> includeExpressions, Tid id, CancellationToken cancellationToken = default)
        {
            ...
            #pragma warning disable CS8602 // Dereference of a possibly null reference.
                        var data = await query.SingleOrDefaultAsync(x =>
                        EF.Property<Tid>(x, "Id").Equals(id)
                        , cancellationToken);
